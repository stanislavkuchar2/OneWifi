...
static int get_mac_by_vap_index (wifi_vap_info_map_t *hal_vap_info_map, int vap_index,  mac_address_t mac)
{
    unsigned int j = 0;
...
static int wifidb_vap_config_update_mld_mac()
{
    ...
    /* find M AP (linkID 0) and obtain it's MAC */
    for (i = 0; i < MLD_UNIT_COUNT; i++) {
        memset(mld_addr_map, 0, sizeof(mld_addr_map));
        memset(mlo_mac, 0, sizeof(mac_address_t));

        wifi_util_info_print(WIFI_DB, "%s:%d: Stano Updating MLO MAC for mld_unit %d MAX radios:%d\r\n", __func__, __LINE__, i, MAX_NUM_RADIOS);
        ...
        for (r_idx=0; r_idx < getNumberRadios(); r_idx++) {
            ...
            for (k = 0; k < mgr_vap_info_map->num_vaps; k++) {
                ...
                if (i == 0) { /* Initialise all vap's mld_mac with interface mac */
                    get_mac_by_vap_index(&hal_vap_info_map, vap_config->vap_index, mld_info->mld_addr);
                    wifi_util_info_print(WIFI_DB, "%s:%d: Updating MAC2 radio:%d i:%d vap_index:%d \r\n",
                        __func__, __LINE__, r_idx, k, vap_config->vap_index);
                }
                ...
                if (mld_info->mld_enable && mld_info->mld_id == i) {
                    ...
                    if(mld_info->mld_link_id == 0) { /* check if the link is main MLO link */
                        get_mac_by_vap_index(&hal_vap_info_map, vap_config->vap_index, mlo_mac);
                        wifi_util_info_print(WIFI_DB, "%s:%d: Updating MAC1 radio:%d i:%d vap_index:%d \r\n",
                            __func__, __LINE__, r_idx, k, vap_config->vap_index);
                    }
                }
            }
